...
x <- getURL("https://raw.githubusercontent.com/JDHaser/DS220-Final-Project/master/OSUWiscyTweets.csv")
...
SummarisedTweets <- DayTweets %>% mutate(Time = ifelse(Day == "2017-12-02",  (as.numeric(hms(Time)) / 3600), (as.numeric(hms(Time)) / 3600) + 24))
...
ggplot(SummarisedTweets, aes(x = Time, y = Sentiment)) + geom_line() +
  labs(title = "Sentiment Analysis of the Ohio State Wisconsin Game", x = "Time of Day(Hours)", y = "Average Sentiment") +
...
GameTimeTweets <- SummarisedTweets %>% filter(Time >= 20 & Time <= 24.5)
